...
def options():
    if optionVar.get().lower() == "download":
        ...
        label5 = Label(root, text= "Received at:" +now.strftime("%Y-%m-%d %H:%M:%S"))
        label5.config(font=('helvetica', 10))
        canvas.create_window(600, 430, window=label5)
        op = "download"
        s.send(op.encode())
        #FileName = input("Enter Filename to Download from server : ")
        #print(en.get())
        FileName = en.get()
        fsize = os.path.getsize("/home/pran/noninstantfile/client/file/"+FileName)
        label4 = Label(root, text= "File size:" +str(fsize)+" Bytes")
        label4.config(font=('helvetica', 10))
        canvas.create_window(600, 400, window=label4)
        Data = "xxx"
        while True:
        ...
        op = "upload"
        now = datetime.datetime.now()
        label5 = Label(root, text= "Send at:" +now.strftime("%Y-%m-%d %H:%M:%S"))
        label5.config(font=('helvetica', 10))
        canvas.create_window(600, 430, window=label5)
        #print ("Send at:",now.strftime("%Y-%m-%d %H:%M:%S"))
        s.send(op.encode())
